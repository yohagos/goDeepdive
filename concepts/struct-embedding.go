...
// seamless composition of types. This is not to be confused with 
// //go:embed which is a go directive introduced in Go version 1.16+ 
...
// A container embeds a base. An embedding looks like a field 
// without a name.
...
func StructEmbedding() {
	...
	// Alternatively, we can spell out the full path using the 
	// embedded type name.
	...
	// interface implementations onto other structs. Here we see 
	// that a container now implements the describer interface because 
	// it embeds base.
	var d describer = co
