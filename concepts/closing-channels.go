...
// In this example we’ll use a jobs channel to communicate work to 
// be done from the main() goroutine to a worker goroutine. When we 
// have no more jobs for the worker we’ll close the jobs channel.
func ClosingChannels() {
	...
	// values in the channel have already been received. We use this 
	...
	// the zero value of the underlying type. The optional second 
	// return value is true if the value received was delivered by 
	// a successful send operation to the channel, or false if it was 
	// a zero value generated because the channel is closed and empty.
	_, ok := <-jobs 
