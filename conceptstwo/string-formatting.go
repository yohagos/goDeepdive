...
// Go offers excellent support for string formatting in the printf
// tradition. Here are some examples of common string formatting tasks.

type point struct {
	x, y int
}

func StringFormattings() {
	// Go offers several printing “verbs” designed to format general
	// Go values. For example, this prints an instance of our point
	// struct.
	p := point{1, 2}
	fmt.Printf("struct1: %v\n", p)

	// If the value is a struct, the %+v variant will include the
	...
	// There are many options for formatting integers. Use %d for
	// standard, base-10 formatting.
	...
	// There are also several formatting options for floats. For basic
	...
	// width and precision of the resulting figure. To specify the
	// width of an integer, use a number after the % in the verb.
	// By default the result will be right-justified and padded with
	// spaces.

	fmt.Printf("width1: |%6d|%6d|\n", 12, 345)

	// You can also specify the width of printed floats, though
	// usually you’ll also want to restrict the decimal precision at
	// the same time with the width.precision syntax.
